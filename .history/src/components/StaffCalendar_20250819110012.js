...
function StaffCalendar({ user }) {
  // เริ่มต้นที่เดือนสิงหาคม 2024 (เดือนที่จัดเวร)
  const [currentDate, setCurrentDate] = useState(new Date(2024, 7, 1)); // 7 = สิงหาคม (0-based)
  ...
  const loadScheduleData = async () => {
    try {
      ...
      console.log('📅 Calendar - Total schedules found:', schedulesSnapshot.docs.length);
      ...
      schedulesSnapshot.forEach((doc) => {
        ...
        if (data.month === month && data.year === year) {
          // เก็บข้อมูลทั้งหมดของเดือนนี้ (ไม่ต้องกรองตอนนี้)
          schedules[doc.id] = data;
          console.log('✅ Calendar - Added schedule for current month:', doc.id);
        } else {
