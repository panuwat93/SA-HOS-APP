...
function StaffCalendar({ user }) {
  ...
  const renderShift = (date) => {
    ...
    if (shift) {
      // ใช้ข้อความที่แท้จริงจากตารางเวร
      let shiftColor = '#4CAF50'; // เริ่มต้นสีเขียว
      let shiftText = shift.text; // ใช้ข้อความจริงจากตารางเวร
      
      // กำหนดสีตามประเภทเวร (แต่แสดงข้อความจริง)
      if (shift.text === 'ช' || shift.text.includes('เช้า')) {
        shiftColor = '#4CAF50'; // เขียว
      } else if (shift.text === 'บ' || shift.text.includes('บ่าย')) {
        shiftColor = '#FF9800'; // ส้ม
      } else if (shift.text === 'ด' || shift.text.includes('ดึก')) {
        shiftColor = '#2196F3'; // น้ำเงิน
      } else {
        // สำหรับข้อความอื่นๆ เช่น "O", "MB" ให้ใช้สีเทา
        shiftColor = '#6c757d'; // เทา
      }
          ...
          if (staffId === user.id || staffId === user.uid) {
            ...
            if (typeof shiftData === 'string') {
              try {
                ...
                if (parsedShift.text === 'ช') {
                  shiftText = 'เวรเช้า';
                } else if (parsedShift.text === 'บ') {
                  shiftText = 'เวรบ่าย';
                } else if (parsedShift.text === 'ด') {
                  shiftText = 'เวรดึก';
                } else {
