...
function StaffCalendar({ user }) {
  ...
  const loadScheduleData = async () => {
    try {
      ...
      console.log(`📅 Calendar - Loading schedule data for month: ${month}, year: ${year}`);
      
      // ดึงข้อมูลตารางเวรจาก schedules collection
      const schedulesSnapshot = await getDocs(collection(db, 'schedules'));
      console.log('📅 Calendar - Raw schedules snapshot:', schedulesSnapshot.docs.length, 'documents');
      ...
      schedulesSnapshot.forEach((doc) => {
        ...
        if (data.month === month && data.year === year) {
          ...
          console.log(`✅ Calendar - Added schedule for ${month}/${year}:`, doc.id);
        } else {
          console.log(`❌ Calendar - Skipped schedule (wrong month/year): ${data.month}/${data.year}`);
        }
      });
      
      console.log('📅 Calendar - Final processed schedule data:', schedules);
      console.log('📅 Calendar - Schedule keys:', Object.keys(schedules));
  ...
  const loadTaskData = async () => {
    try {
      console.log('📋 Calendar - Loading task data for user:', user.firstName, user.lastName, user.uid);
      
      // ดึงข้อมูลงานจาก taskAssignments collection (ตามที่ใช้ใน TaskAssignment)
      const tasksSnapshot = await getDocs(collection(db, 'taskAssignments'));
      console.log('📋 Calendar - Raw tasks snapshot:', tasksSnapshot.docs.length, 'documents');
      
      const tasks = {};
      
      tasksSnapshot.forEach((doc) => {
        const data = doc.data();
        console.log(`📋 Calendar - Task document ${doc.id}:`, data);
        
        if (data.assignments) {
          console.log(`📋 Calendar - Processing assignments for date: ${data.date}`);
          
          // ตรวจสอบงานในแต่ละกะ
          Object.entries(data.assignments).forEach(([shiftId, shiftData]) => {
            console.log(`📋 Calendar - Processing shift: ${shiftId}`);
            
            Object.entries(shiftData).forEach(([bedId, bedData]) => {
              console.log(`📋 Calendar - Processing bed: ${bedId}`, bedData);
              ...
              if (bedData.staff) {
                ...
                console.log(`📋 Calendar - Staff check: ${bedData.staff} vs ${user.firstName} ${user.lastName} (${user.uid}) - Result: ${isUserTask}`);
                ...
                if (isUserTask) {
                  ...
                  console.log(`✅ Calendar - Added task for date ${date}:`, bedData);
                }
              }
            });
          });
      ...
      console.log('📋 Calendar - Final processed task data:', tasks);
      console.log('📋 Calendar - Task dates:', Object.keys(tasks));
  ...
  const renderShift = (date) => {
    ...
    // Debug: แสดงข้อมูลที่กำลังค้นหา
    console.log(`🔍 Calendar - Searching for shift on ${day}/${month}/${year} for user: ${user.firstName} ${user.lastName}`);
    console.log('🔍 Calendar - Current scheduleData:', scheduleData);
    ...
    Object.entries(scheduleData).forEach(([scheduleId, schedule]) => {
      console.log(`🔍 Calendar - Checking schedule:`, scheduleId, schedule);
      
      if (schedule.month === month && schedule.year === year && schedule.shifts) {
        console.log(`🔍 Calendar - Found matching month/year, checking day ${day}`);
        console.log(`🔍 Calendar - Day shifts:`, schedule.shifts[day]);
        
        // ค้นหาจากวันที่และชื่อ-นามสกุล
        const dayShifts = schedule.shifts[day];
        if (dayShifts) {
          Object.entries(dayShifts).forEach(([staffId, shiftData]) => {
            console.log(`🔍 Calendar - Checking staff ${staffId}:`, shiftData);
            
            // ตรวจสอบจากชื่อ-นามสกุล
            if (typeof shiftData === 'string' && 
                shiftData.includes(`${user.firstName} ${user.lastName}`)) {
              shift = shiftData;
              shiftInfo = { staffId, shiftData };
              console.log(`✅ Calendar - Found shift!`, shift);
            }
          });
        }
      }
    ...
    if (shift) {
      ...
      if (shift.includes('ช') || shift.includes('เช้า') || shift.includes('Morning')) {
        shiftColor = '#4CAF50'; // เขียว
        shiftText = 'เช้า';
      } else if (shift.includes('บ') || shift.includes('บ่าย') || shift.includes('Afternoon')) {
        shiftColor = '#FF9800'; // ส้ม
        shiftText = 'บ่าย';
      } else if (shift.includes('ด') || shift.includes('ดึก') || shift.includes('Night')) {
        shiftColor = '#2196F3'; // น้ำเงิน
        shiftText = 'ดึก';
      }
      
      console.log(`📅 Calendar - Rendering shift: ${shiftText} (${shift})`);
    ...
    console.log(`❌ Calendar - No shift found for ${user.firstName} ${user.lastName} on ${day}/${month}/${year}`);
    return null;
  };
