...
function OnCallSchedule({ user }) {
  ...
  const canEdit = () => {
    ...
    console.log('🔐 ตรวจสอบสิทธิ์:', { userRole: user?.role, isAdmin });
    return isAdmin;
  };
  ...
  const handleCellInput = (day, staffId, value) => {
    ...
    if (!canEdit()) {
      console.log('❌ ไม่มีสิทธิ์แก้ไขตาราง On Call - เฉพาะแอดมินเท่านั้น');
      return; // ไม่มีสิทธิ์แก้ไข
    }
    
    console.log(`📝 อัพเดทเซลล์: วันที่ ${day}, เจ้าหน้าที่ ${staffId}, ค่า: ${value}`);
    ...
    setScheduleData(prev => {
      ...
      console.log('📊 ข้อมูลใหม่:', newData);
      return newData;
    });
  };
