...
function ScheduleManagement({ user }) {
  ...
  const saveDraft = async () => {
    try {
      // แยก draftId ตาม role และ user
      const userIdentifier = user?.role === 'admin' ? 'admin' : `staff_${user?.uid}`;
      const draftId = `draft_${currentYear}_${currentMonth + 1}_${userIdentifier}`;
