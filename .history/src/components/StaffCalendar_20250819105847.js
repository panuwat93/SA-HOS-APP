      ...
      if (schedule.month === month && schedule.year === year && schedule.shifts) {
        ...
        if (dayShifts) {
          if (day === 1) { // แสดง debug เฉพาะวันแรก
            console.log(`🔍 Calendar - Day ${day} shifts:`, dayShifts);
            console.log(`🔍 Calendar - All staff IDs for day ${day}:`, Object.keys(dayShifts));
          }
          ...
          if (typeof shiftData === 'string') {
            ...
            if (staffId === user.id || staffId === user.uid) {
              ...
              console.log(`❌ Calendar - Staff ID mismatch: ${staffId} vs ${user.id || user.uid}`);
              console.log(`❌ Calendar - User not found in staff list for day ${day}`);
            }
          }
        });
        }
