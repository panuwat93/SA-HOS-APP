    ...
    try {
      ...
      schedulesSnapshot.forEach((doc) => {
        ...
        if (data.month === month && data.year === year) {
          ...
          if (data.shifts) {
            Object.entries(data.shifts).forEach(([day, dayShifts]) => {
              // ตรวจสอบว่าวันนี้มีเวรของ user นี้หรือไม่
              if (dayShifts && (dayShifts[user.id] || dayShifts[user.uid])) {
                filteredShifts[day] = {
                  [user.id || user.uid]: dayShifts[user.id || user.uid]
                };
              }
            });
          }
          
          // เก็บเฉพาะข้อมูลที่กรองแล้ว
          schedules[doc.id] = {
            ...data,
            shifts: filteredShifts
          };
          console.log('✅ Calendar - Added filtered schedule for current month:', doc.id);
        }
      });
      
      console.log('📅 Calendar - Final schedules for', month, '/', year, ':', Object.keys(schedules));
      console.log('📅 Calendar - Filtered schedule data for user:', schedules);
