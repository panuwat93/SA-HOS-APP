...
function StaffManagement() {
  ...
  const createAccountForStaff = async (staff) => {
    try {
      const { username, password } = generateCredentials(staff.firstName, staff.lastName);
      
      if (window.confirm(`คุณต้องการสร้างบัญชีให้ ${staff.firstName} ${staff.lastName} หรือไม่?\n\nUsername: ${username}\nรหัสผ่าน: ${password}\n\n⚠️ กรุณาบันทึกข้อมูลนี้ไว้`)) {
        ...
        const newUser = {
          ...
          firstName: staff.firstName,
          lastName: staff.lastName,
          department: staff.department || currentAdmin?.department || 'หอผู้ป่วยศัลยกรรมอุบัติเหตุ',
          position: staff.position,
          role: 'staff',
          createdAt: new Date().toISOString(),
          // เก็บรหัสผ่านเริ่มต้นไว้ใน Firestore (ไม่แนะนำในระบบจริง)
            ...
            <div>
              ...
              <div style={{ 
                ...
                <div style={{ 
                  ...
                  💡 ข้อมูลจะถูกดึงจาก Firebase collections จริง และอัพเดทแบบเรียลไทม์
                </div>
              </div>
                        ...
                        <td style={{ padding: '8px', border: '1px solid #dee2e6', fontFamily: 'monospace' }}>
                          {staff.userExists ? (
                            ...
                            <span style={{ color: '#856404', fontStyle: 'italic' }}>
                              {generateCredentials(staff.firstName, staff.lastName).username}
                            </span>
                          )}
                        </td>
                        <td style={{ padding: '8px', border: '1px solid #dee2e6', fontFamily: 'monospace' }}>
                          {staff.userExists ? (
                            ...
                            <span style={{ color: '#856404', fontStyle: 'italic' }}>
                              {generateCredentials(staff.firstName, staff.lastName).password}
                            </span>
                          )}
                        </td>
                        <td style={{ padding: '8px', border: '1px solid #dee2e6' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            ...
                            {staff.userData && (
                              <span 
                                ...
                                onClick={() => console.log('Debug data for', staff.firstName, staff.lastName, ':', staff.userData)}
                              >
                                🔍
                              </span>
                            )}
              ...
              <div style={{ 
                ...
                <ul style={{ margin: '5px 0', paddingLeft: '20px' }}>
                  ...
                  <li>สำหรับเจ้าหน้าที่ที่ไม่มีบัญชี สามารถกดปุ่ม "สร้างบัญชี" เพื่อสร้างบัญชีใหม่</li>
                  <li>Username จะถูกสร้างจากชื่อ-นามสกุล และรหัสผ่านจะเป็นรหัสสุ่ม 6 ตัวอักษร</li>
                  <li>หากพบปัญหา ให้ตรวจสอบ Console ใน Developer Tools เพื่อดูข้อมูลที่ถูกดึงมา</li>
                  <li>คลิกไอคอน 🔍 เพื่อดูข้อมูล debug ของแต่ละเจ้าหน้าที่</li>
                  <li>ข้อมูลจะถูกดึงจาก Firebase collections: 'staff' และ 'users'</li>
                  <li>หากต้องการสร้างบัญชีใหม่ ให้กดปุ่ม "สร้างบัญชี" ในคอลัมน์สถานะ</li>
                  <li>Username จะถูกสร้างในรูปแบบ: ชื่อนามสกุล (เช่น: duangkaewsomphool)</li>
                  <li>รหัสผ่านเริ่มต้นจะเป็นรหัสสุ่ม 6 ตัวอักษร (เช่น: a1b2c3)</li>
                  <li>หลังจากสร้างบัญชีแล้ว ให้รีเฟรชข้อมูลเพื่อดูข้อมูลล่าสุด</li>
                  <li>ข้อมูลจะถูกเก็บใน Firebase collections: 'users' และ 'staff'</li>
                  <li>รหัสผ่านจะถูกอัพเดทใน Firestore เมื่อเจ้าหน้าที่เปลี่ยนรหัสผ่าน</li>
                  <li>ข้อมูลในตารางจะตรงกับรหัสผ่านจริงที่เจ้าหน้าที่ใช้ล็อกอิน</li>
                </ul>
              </div>
            </div>
          )}
