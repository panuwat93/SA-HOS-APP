...
function StaffCalendar({ user }) {
  ...
  const renderShift = (date) => {
    ...
    console.log('🔍 Calendar - Rendering shift for date:', dateStr, 'day:', day);
    console.log('🔍 Calendar - User:', user.firstName, user.lastName);
    console.log('🔍 Calendar - Schedule data:', scheduleData);
    
    // ค้นหาเวรใน scheduleData
    let shift = null;
    Object.values(scheduleData).forEach(schedule => {
      if (schedule.shifts && schedule.shifts[day]) {
        console.log('🔍 Calendar - Found shifts for day:', day, schedule.shifts[day]);
        
        // ค้นหาจากชื่อ-นามสกุลใน shifts ของวันนั้น
        const dayShifts = schedule.shifts[day];
        Object.values(dayShifts).forEach(shiftData => {
          console.log('🔍 Calendar - Checking shift data:', shiftData);
          
          if (typeof shiftData === 'string' && 
              shiftData.includes(`${user.firstName} ${user.lastName}`)) {
            shift = shiftData;
            console.log('🔍 Calendar - Found matching shift:', shift);
          }
        });
      }
    });
