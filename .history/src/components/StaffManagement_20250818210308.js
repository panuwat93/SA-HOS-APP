...
function StaffManagement() {
  ...
  const loadStaffLoginInfo = async () => {
    try {
      ...
      // ดึงข้อมูลผู้ใช้ (users collection)
      ...
      const combinedInfo = staffData.map(staff => {
        const userInfo = usersData.find(user => 
          user.email === `${staff.firstName.toLowerCase()}.${staff.lastName.toLowerCase()}@hospital.com` ||
          user.email === `${staff.firstName.toLowerCase()}${staff.lastName.toLowerCase()}@hospital.com` ||
          user.email === `${staff.firstName.toLowerCase()}_${staff.lastName.toLowerCase()}@hospital.com`
        );
        ...
        return {
          ...
          lastPasswordChange: userInfo?.lastPasswordChange || 'ไม่พบข้อมูล',
          userExists: !!userInfo
        };
      });
