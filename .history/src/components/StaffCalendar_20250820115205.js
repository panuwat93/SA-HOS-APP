...
function StaffCalendar({ user }) {
  ...
  const renderShift = (date) => {
    ...
    if (shifts.length > 0) {
      // เรียงลำดับเวรตามแถว: แถวบน (ไม่มี _extra) ก่อน, แถวล่าง (_extra) หลัง
      const sortedShifts = shifts.sort((a, b) => {
        const aIsExtra = a.staffId.includes('_extra');
        const bIsExtra = b.staffId.includes('_extra');
        
        if (aIsExtra && !bIsExtra) return 1;  // a เป็นแถวล่าง, b เป็นแถวบน → b ก่อน
        if (!aIsExtra && bIsExtra) return -1; // a เป็นแถวบน, b เป็นแถวล่าง → a ก่อน
        return 0; // ทั้งคู่เป็นแถวเดียวกัน
      });
