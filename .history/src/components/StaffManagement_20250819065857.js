...
function StaffManagement() {
  ...
  const handleCreateAccount = async (username, password) => {
    try {
      ...
      const email = `${username}@hospital.local`; // สร้าง email จาก username
      
      // ใช้ Firebase Admin SDK หรือ Cloud Functions สำหรับสร้างบัญชี
      // แต่เนื่องจากเราใช้ client-side เราจะสร้างบัญชีด้วย createUserWithEmailAndPassword
      // และลบออกทันทีเพื่อให้แอดมินสามารถสร้างบัญชีได้
      
      // สร้างข้อมูลผู้ใช้ใน Firestore พร้อม email และ password
      const newUser = {
        username: username,
        email: email,
      ...
      alert(`✅ สร้างบัญชีสำเร็จ!\n\nUsername: ${username}\nEmail: ${email}\nรหัสผ่าน: ${password}\n\n⚠️ กรุณาบันทึกข้อมูลนี้ไว้และแจ้งเจ้าหน้าที่\n\n💡 เจ้าหน้าที่สามารถล็อกอินด้วย Email และรหัสผ่านได้เลย!`);
