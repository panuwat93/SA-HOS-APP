...
function StaffCalendar({ user }) {
  ...
  useEffect(() => {
    if (user) {
      loadScheduleData();
      loadTaskData();
      loadStaffData(); // โหลดข้อมูลพยาบาล
    }
  }, [user, currentDate]);
  ...
  const renderShift = (date) => {
    ...
    if (shift) {
      ...
      // หาข้อมูลงานที่มอบหมายสำหรับวันนี้
      let assignmentInfo = '';
      const dayTasks = taskData[dateStr];
      
      if (dayTasks) {
        // ตรวจสอบว่า user เป็นพยาบาลหรือผู้ช่วย
        const userStaff = staffList.find(s => s.id === user.id || s.id === user.uid);
        
        if (userStaff) {
          if (userStaff.position === 'พยาบาล') {
            // สำหรับพยาบาล: หาเตียงที่ได้รับมอบหมาย
            Object.entries(dayTasks).forEach(([shiftId, shiftData]) => {
              if (shiftData.staffRow && shiftData.staffRow[user.id || user.uid]) {
                const staffData = shiftData.staffRow[user.id || user.uid];
                if (staffData.assignedBed) {
                  assignmentInfo = `เตียง: ${staffData.assignedBed}`;
                }
              }
            });
          } else if (['ผู้ช่วยพยาบาล', 'ผู้ช่วยเหลือคนไข้', 'Part time'].includes(userStaff.position)) {
            // สำหรับผู้ช่วย: หาทีมที่ได้รับมอบหมาย
            Object.entries(dayTasks).forEach(([shiftId, shiftData]) => {
              if (shiftData.assistantRow && shiftData.assistantRow[user.id || user.uid]) {
                const assistantData = shiftData.assistantRow[user.id || user.uid];
                if (assistantData.assistantTeam) {
                  const teamLabel = assistantData.assistantTeam === 'team_a' ? 'TEAM A' : 'TEAM B';
                  assignmentInfo = `ทีม: ${teamLabel}`;
                }
              }
            });
          }
