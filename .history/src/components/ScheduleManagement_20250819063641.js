...
function ScheduleManagement({ user }) {
  ...
  const insertShiftToSelectedCells = (shiftType, color, backgroundColor = 'transparent') => {
    ...
    try {
      ...
      const bgName = backgroundColor === '#FFFFFF' ? 'ขาว' : backgroundColor === '#FF0000' ? 'แดง' : 'โปร่งใส';
      
      let message = '';
      if (shiftType === 'O' || shiftType === 'VA') {
        message = `ใส่ ${shiftType} อักษรสี${colorName} พื้น${bgName} ใน ${selectedCells.length} ช่องเรียบร้อยแล้ว`;
      } else {
        message = `ใส่เวร${shiftName}สี${colorName} ใน ${selectedCells.length} ช่องเรียบร้อยแล้ว`;
      }
          ...
          <div className="formatting-toolbar">
            ...
            <div className="toolbar-section">
              ...
              <button
                ...
                className="format-btn shift-btn va-red"
                title="ใส่ VA อักษรดำพื้นแดง"
                disabled={!user?.canEditSchedule || selectedCells.length === 0}
              >
                VA
              </button>
        ...
        <div className="schedule-section">
          <div className="formatting-toolbar">
            ...
            <div className="toolbar-section">
              <span className="toolbar-label">อื่นๆ:</span>
              <button
                onClick={() => user?.canEditSchedule ? insertShiftToSelectedCells('O', '#FF0000', '#FFFFFF') : null}
                className="format-btn shift-btn o-white"
                title="ใส่ O อักษรแดงพื้นขาว"
                disabled={!user?.canEditSchedule || selectedCells.length === 0}
              >
                O
              </button>
