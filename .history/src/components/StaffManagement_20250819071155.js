...
function StaffManagement() {
  ...
  const loadStaffLoginInfo = async () => {
    try {
      ...
      const combinedInfo = staffData.map(staff => {
        ...
        // วิธีที่ 6: ตรวจสอบว่ามี UID ใน Firebase Auth หรือไม่ (สำคัญที่สุด)
        if (userInfo && userInfo.uid) {
          // ตรวจสอบว่า UID นี้มีอยู่ใน Firebase Auth จริงๆ หรือไม่
          try {
            // ลองดึงข้อมูลจาก Firebase Auth
            const authUser = await getDoc(doc(db, 'users', userInfo.uid));
            if (!authUser.exists()) {
              // ถ้าไม่มีใน Firestore users collection ให้ถือว่าไม่มีบัญชี
              userInfo = null;
            }
          } catch (error) {
            // ถ้าเกิด error ให้ถือว่าไม่มีบัญชี
            userInfo = null;
          }
        }
  ...
  const createAccountForStaff = async (staff) => {
    try {
      ...
      if (staff.userExists && staff.uid) {
        // ถ้ามีบัญชีอยู่แล้ว ให้ตรวจสอบว่าสามารถสร้างใหม่ได้หรือไม่
        const canCreateNew = window.confirm(
          `เจ้าหน้าที่ ${staff.firstName} ${staff.lastName} มีบัญชีอยู่แล้ว\n\n` +
          `ต้องการสร้างบัญชีใหม่แทนหรือไม่?\n\n` +
          `⚠️ หมายเหตุ: บัญชีเก่าจะถูกแทนที่ด้วยบัญชีใหม่`
        );
  ...
  const handleCreateAccount = async (username, password) => {
    try {
      ...
      // ตรวจสอบว่า Username ซ้ำหรือไม่ (ยกเว้นเจ้าหน้าที่คนนี้)
      const usersSnapshot = await getDocs(collection(db, 'users'));
      const existingUser = usersSnapshot.docs.find(doc => {
        const userData = doc.data();
        // ถ้าเป็นเจ้าหน้าที่คนเดียวกัน ให้ข้ามไป
        if (userData.firstName === creatingStaff.firstName && 
            userData.lastName === creatingStaff.lastName) {
          return false;
        }
      ...
      if (existingUser) {
        ...
        try {
          // ลบบัญชีเก่าถ้ามีอยู่แล้ว
          if (creatingStaff.uid) {
            try {
              // ลบข้อมูลใน Firestore users collection
              await deleteDoc(doc(db, 'users', creatingStaff.uid));
              console.log('✅ ลบบัญชีเก่าใน Firestore เรียบร้อย');
            } catch (deleteError) {
              console.log('⚠️ ไม่สามารถลบบัญชีเก่าได้:', deleteError.message);
            }
          }
      ...
      <div className="modal-content login-info-modal" style={{ maxWidth: '1000px', width: '95%' }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            ...
                            {!staff.userExists ? (
                              ...
                              <button
                                ...
                                className="btn btn-warning"
                                style={{ fontSize: '12px', padding: '4px 8px' }}
                                title="สร้างบัญชีใหม่แทนที่บัญชีเก่า"
                              >
                                🔄 สร้างใหม่
                              </button>
                            )}
        ...
        <div className="modal-footer">
          ...
          <div style={{ 
            ...
            🔄 หากข้อมูลไม่ตรงกัน ให้กดปุ่ม "รีเฟรชข้อมูล" เพื่ออัพเดทข้อมูลล่าสุด
          </div>
