  ...
  const loadStaffLoginInfo = async () => {
    try {
      ...
      const combinedInfo = staffData.map(staff => {
        ...
        // วิธีที่ 1: ค้นหาจาก email ที่ตรงกับชื่อ-นามสกุล
        ...
        // วิธีที่ 3: ค้นหาจาก firstName และ lastName ใน user data
        if (!userInfo) {
          userInfo = usersData.find(user => {
        ...
        return {
          ...staff,
          username: userInfo?.email || 'ไม่พบข้อมูล',
...
// โมดัลข้อมูลล็อกอิน
function LoginInfoModal({ onClose, staffLoginInfo, loginInfoLoading, onRefresh }) {
  return (
    <div className="modal">
      <div className="modal-content login-info-modal" style={{ maxWidth: '1000px', width: '95%' }}>
        ...
        <div className="modal-body">
          {loginInfoLoading ? (
            ...
            <div>
              <div style={{ marginBottom: '15px', color: '#856404', fontSize: '14px' }}>
                ข้อมูลล็อกอินของเจ้าหน้าที่ทั้งหมด {staffLoginInfo.length} คน
              </div>
                ...
                <table style={{ 
                  ...
                  <thead>
                    <tr style={{ background: '#f8f9fa' }}>
                      ...
                      <th style={{ padding: '8px', border: '1px solid #dee2e6', textAlign: 'left' }}>อัพเดทล่าสุด</th>
                      <th style={{ padding: '8px', border: '1px solid #dee2e6', textAlign: 'left' }}>สถานะ</th>
                      <th style={{ padding: '8px', border: '1px solid #dee2e6', textAlign: 'left' }}>ข้อมูลเพิ่มเติม</th>
                    </tr>
                  </thead>
                  <tbody>
                    {staffLoginInfo.map((staff, index) => (
                      <tr key={staff.id} style={{ 
                        ...
                        <td style={{ padding: '8px', border: '1px solid #dee2e6', fontSize: '10px' }}>
                          {staff.userExists ? (
                            <div>
                              <div>ID: {staff.userId}</div>
                              <div>Email: {staff.userData?.email || 'ไม่พบ'}</div>
                              <div>Display: {staff.userData?.displayName || 'ไม่พบ'}</div>
                            </div>
                          ) : (
                            <span style={{ color: '#6c757d' }}>ไม่มีข้อมูล</span>
                          )}
                        </td>
                      </tr>
                    ))}
              ...
              <div style={{ 
                ...
                <ul style={{ margin: '5px 0', paddingLeft: '20px' }}>
                  ...
                  <li>หากเจ้าหน้าที่เปลี่ยนรหัสผ่าน จะแสดงรหัสผ่านล่าสุดทันที</li>
              ...
              <div style={{ 
                ...
                <div style={{ marginTop: '5px' }}>
                  <button 
                    onClick={() => {
                      console.log('🔍 Debug: ข้อมูลเจ้าหน้าที่ทั้งหมด:', staffLoginInfo);
                      console.log('🔍 Debug: ข้อมูลผู้ใช้ที่มีบัญชี:', staffLoginInfo.filter(s => s.userExists));
                      console.log('🔍 Debug: ข้อมูลผู้ใช้ที่ไม่มีบัญชี:', staffLoginInfo.filter(s => !s.userExists));
                    }}
