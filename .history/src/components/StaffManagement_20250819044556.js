  ...
  const loadStaffLoginInfo = async () => {
    try {
      ...
      const combinedInfo = staffData.map(staff => {
        ...
        // วิธีที่ 3: ค้นหาจาก displayName หรือชื่อที่แสดง
        if (!userInfo) {
          userInfo = usersData.find(user => {
        ...
        // วิธีที่ 4: ค้นหาจาก email ที่ตรงกับชื่อ-นามสกุล (กรณีพิเศษ)
        if (!userInfo) {
          userInfo = usersData.find(user => {
        ...
        const debugInfo = {
          ...
          foundUser: userInfo ? {
            ...
            passwordLength: userInfo.password?.length || 0
          } : null,
          searchMethods: [
            'firstName + lastName',
            'username pattern',
        ...
        console.log(`🔍 Debug info for ${staff.firstName} ${staff.lastName}:`, debugInfo);
        ...
        return {
          ...
          debugInfo: debugInfo // เก็บข้อมูล debug เพิ่มเติม
        };
      });
      ...
      <div className="modal-content login-info-modal" style={{ maxWidth: '1000px', width: '95%' }}>
        ...
        <div className="modal-header">
          ...
          <div style={{ 
            ...
            Debug: staffLoginInfo.length = {staffLoginInfo.length} | filteredData.length = {filteredData.length}
          </div>
        </div>
            ...
            <div>
              ...
              <div style={{ 
                ...
                <div style={{ 
                  ...
                  หน้า {currentPage + 1} จาก {totalPages} | แสดง {currentData.length} รายการ
                </div>
                <div style={{ 
                  display: 'flex', 
                  gap: '15px', 
                ...
                {currentData.length === 0 ? (
                  <div style={{ 
                    ...
                    <div style={{ fontSize: '12px', marginTop: '10px' }}>
                      staffLoginInfo: {staffLoginInfo.length} | filteredData: {filteredData.length} | currentData: {currentData.length}
                    </div>
                  </div>
                ) : (
                  <table style={{ 
                            ...
                            {staff.userData && (
                              <span 
                                ...
                                onClick={() => {
                                  ...
                                  console.log('User Data:', staff.userData);
                                  console.log('Debug Info:', staff.debugInfo);
                                  console.log('Has Password:', !!staff.userData.password);
                                  console.log('Password Length:', staff.userData.password?.length || 0);
                                  console.log('==================');
                                }}
                              >
                                🔍
