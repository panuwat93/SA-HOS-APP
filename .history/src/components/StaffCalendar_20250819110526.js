...
function StaffCalendar({ user }) {
  ...
  const loadScheduleData = async () => {
    try {
      ...
      console.log('📅 Calendar - Loading data for month:', month, 'year:', year);
      ...
      schedulesSnapshot.forEach((doc) => {
        const data = doc.data();
        console.log('📅 Calendar - Found schedule:', doc.id, 'month:', data.month, 'year:', data.year);
        
        // กรองเฉพาะเดือน/ปีที่ต้องการ
        if (data.month === month && data.year === year) {
          schedules[doc.id] = data;
          console.log('✅ Calendar - Added schedule:', doc.id, 'for month:', data.month, 'year:', data.year);
        } else {
          console.log('❌ Calendar - Skipped schedule (wrong month/year):', doc.id, 'month:', data.month, 'year:', data.year);
        }
      });
      
      console.log('📅 Calendar - Final schedules for', month, '/', year, ':', Object.keys(schedules));
      console.log('📅 Calendar - Schedule data:', schedules);
  ...
  const renderShift = (date) => {
    ...
    // Debug: แสดงข้อมูลที่กำลังค้นหา (เฉพาะวันแรกของเดือน)
    if (day === 1) {
      console.log(`🔍 Calendar - Searching for shift on ${day}/${month}/${year}`);
      console.log('🔍 Calendar - Current scheduleData keys:', Object.keys(scheduleData));
      console.log('🔍 Calendar - scheduleData:', scheduleData);
      console.log('🔍 Calendar - User ID to search for:', user.id || user.uid);
      console.log('🔍 Calendar - Current date state:', currentDate);
      console.log('🔍 Calendar - Current month/year from state:', currentDate.getMonth() + 1, currentDate.getFullYear());
    }
    ...
    Object.entries(scheduleData).forEach(([scheduleId, schedule]) => {
      if (schedule.month === month && schedule.year === year && schedule.shifts) {
        ...
        Object.entries(dayShifts).forEach(([staffId, shiftData]) => {
          if (day === 1) { // แสดง debug เฉพาะวันแรก
            console.log(`🔍 Calendar - Checking staff ${staffId} vs user ${user.id || user.uid}:`, shiftData);
          }
          ...
          if (typeof shiftData === 'string') {
            ...
            if (staffId === user.id || staffId === user.uid) {
              ...
              if (day === 1) { // แสดงเฉพาะวันแรก
                console.log(`✅ Calendar - Found shift for user on ${day}/${month}/${year}:`, parsedShift);
              }
            }
          }
        });
    ...
    if (shift) {
      ...
      if (day === 1) {
        console.log(`🎨 Calendar - Rendering shift: ${shiftText} with colors:`, {
          backgroundColor,
          textColor,
    ...
    // Debug: ถ้าไม่เจอเวร
    if (day === 1) {
      console.log(`❌ Calendar - No shift found for user on ${day}/${month}/${year}`);
      console.log('❌ Calendar - User ID being searched:', user.id || user.uid);
      console.log('❌ Calendar - Available staff IDs:', Object.keys(scheduleData).map(scheduleId => {
        const schedule = scheduleData[scheduleId];
        if (schedule.month === month && schedule.year === year && schedule.shifts) {
          return Object.keys(schedule.shifts[day] || {});
        }
        return [];
      }).flat());
    }
  ...
  const changeMonth = (direction) => {
    console.log('🔄 Calendar - Changing month:', direction);
    setCurrentDate(prev => {
      ...
      console.log('🔄 Calendar - New date:', newDate.toLocaleDateString('th-TH'));
      return newDate;
    });
  };
