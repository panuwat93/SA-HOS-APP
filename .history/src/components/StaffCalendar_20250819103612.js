...
function StaffCalendar({ user }) {
  ...
  const loadScheduleData = async () => {
    try {
      ...
      console.log(`📅 Calendar - Loading schedule for month: ${month}, year: ${year}`);
      
      // ดึงข้อมูลตารางเวรจาก schedules collection
      const schedulesSnapshot = await getDocs(collection(db, 'schedules'));
      console.log('📅 Calendar - Found schedules:', schedulesSnapshot.docs.length, 'documents');
      
      const schedules = {};
      
      schedulesSnapshot.forEach((doc) => {
        const data = doc.data();
        console.log(`📅 Calendar - Document ${doc.id}:`, {
          month: data.month,
          year: data.year,
          hasShifts: !!data.shifts,
          shiftsKeys: data.shifts ? Object.keys(data.shifts) : 'none'
        });
      ...
      console.log('📅 Calendar - Final schedules:', Object.keys(schedules));
  ...
  const renderShift = (date) => {
    ...
    if (Object.keys(scheduleData).length === 0) {
      console.log('📅 Calendar - No schedule data available');
      return null;
    }
    ...
    Object.entries(scheduleData).forEach(([scheduleId, schedule]) => {
      if (schedule.month === month && schedule.year === year && schedule.shifts) {
        ...
        if (dayShifts) {
          console.log(`🔍 Calendar - Checking day ${day} shifts:`, dayShifts);
          
          Object.entries(dayShifts).forEach(([staffId, shiftData]) => {
            console.log(`🔍 Calendar - Staff ${staffId}:`, shiftData);
            ...
            if (typeof shiftData === 'string' && 
              ...
              console.log(`✅ Calendar - Found shift for ${user.firstName} ${user.lastName}:`, shift);
            }
          });
        }
      }
