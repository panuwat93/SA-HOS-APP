...
function StaffCalendar({ user }) {
  ...
  const renderShift = (date) => {
    ...
    if (shift) {
      // ใช้ข้อความและสีจากตารางเวรโดยตรง
      let shiftText = shift.text; // ใช้ข้อความจริงจากตารางเวร
      let shiftColor = shift.color || '#6c757d'; // ใช้สีจากตารางเวร หรือสีเทาเป็น default
      let textColor = 'white'; // สีข้อความ
      
      // ถ้าสีพื้นหลังเป็นสีเข้ม ให้ใช้สีข้อความเป็นขาว
      // ถ้าสีพื้นหลังเป็นสีอ่อน ให้ใช้สีข้อความเป็นดำ
      if (shift.color && (shift.color === '#000000' || shift.color === '#FF0000')) {
        textColor = 'white';
      } else if (shift.color && (shift.color === '#FFFF00' || shift.color === '#00FF00')) {
        textColor = 'black';
      }
      ...
      return (
        <div 
          ...
          title={`เวร: ${shift.text} (สี: ${shift.color})`}
        >
          {shiftText}
        </div>
      );
