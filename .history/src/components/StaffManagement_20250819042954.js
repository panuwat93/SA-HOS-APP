...
function StaffManagement() {
  ...
  // ฟังก์ชันรีเซ็ตรหัสผ่านให้เจ้าหน้าที่ที่มีบัญชีแล้ว
  const resetPasswordForStaff = async (staff) => {
    try {
      const { username, password } = generateCredentials(staff.firstName, staff.lastName);
      
      if (window.confirm(`คุณต้องการรีเซ็ตรหัสผ่านให้ ${staff.firstName} ${staff.lastName} หรือไม่?\n\nUsername: ${staff.username}\nรหัสผ่านใหม่: ${password}\n\n⚠️ กรุณาบันทึกข้อมูลนี้ไว้และแจ้งเจ้าหน้าที่`)) {
        
        // อัพเดทรหัสผ่านใน Firestore
        await updateDoc(doc(db, 'users', staff.userId), {
          password: password,
          updatedAt: new Date().toISOString()
        });
      ...
      <div className="table-container">
       ...
       {showLoginInfoModal && (
         <LoginInfoModal
           ...
           resetPasswordForStaff={resetPasswordForStaff}
         />
       )}
    </div>
  );
...
// โมดัลข้อมูลล็อกอิน
function LoginInfoModal({ onClose, staffLoginInfo, loginInfoLoading, onRefresh, currentPage, setCurrentPage, searchLoginTerm, setSearchLoginTerm, itemsPerPage, generateCredentials, createAccountForStaff, resetPasswordForStaff }) {
            ...
            <div>
              ...
              <div style={{ 
                ...
                <div style={{ 
                  ...
                  🔐 รหัสผ่านจะถูกเก็บไว้ใน Firestore และแสดงในตาราง
                </div>
              </div>
