...
function TaskAssignment({ user }) {
  ...
  // ข้อมูลหน้าที่
  const duties = [
    { value: 'registration', label: 'ลงทะเบียน', color: '#ffffff' },
    { value: 'bed_booking', label: 'จองเตียง', color: '#ffffff' },
    { value: 'registration_bed', label: 'ลงทะเบียน / จองเตียง', color: '#ffffff' },
    { value: 'pipeline', label: 'Pipe line', color: '#ffffff' },
    { value: 'emergency_vehicle', label: 'รถ Emergency', color: '#ffffff' },
    { value: 'pipeline_emergency', label: 'Pipe line / รถ Emergency', color: '#ffffff' },
    { value: 'medicine', label: 'ยา Stock', color: '#ffffff' },
    { value: 'productivity', label: 'Productivity', color: '#ffffff' },
    { value: 'check_delfib', label: 'Check Delfib', color: '#ffffff' },
    { value: 'productivity_check', label: 'Productivity / Check Delfib', color: '#ffffff' }
  ];
  ...
  // ฟังก์ชันเลือก ERT อัตโนมัติตามทีม
  const getDefaultERTForTeam = (teamValue) => {
    if (teamValue === 'team_a') {
      return 'life_support'; // TEAM A = เคลื่อนย้ายกู้ชีพ
    } else if (teamValue === 'team_b') {
      return 'technician'; // TEAM B = ช่างและเส้นทาง
    }
    return '';
  };
  ...
  // ข้อมูลทีม
  const teams = [
    { value: 'team_a', label: 'TEAM A', color: '#ffffff' },
    { value: 'team_b', label: 'TEAM B', color: '#ffffff' }
  ];
        ...
        <div className="header-controls">
          <div className="date-navigation">
            <div className="date-picker-container">
              <label htmlFor="date-picker-readonly" className="selector-label">วันที่:</label>
              <input
                type="date"
  ...
  return (
    <div className="dashboard-content">
      ...
      <div className="task-controls">
        <div className="date-navigation">
          <div className="date-picker-container">
            <label htmlFor="date-picker" className="selector-label">วันที่:</label>
            <input
              type="date"
              id="date-picker"
              value={currentDate.toISOString().split('T')[0]}
                                 ...
                                 <span className="staff-name">{staff.firstName} {staff.lastName}</span>
                                   ...
                                   <select
                                     ...
                                     onChange={(e) => {
                                       ...
                                       // อัพเดททีม
                                       updateSimpleTaskData(shift.id, staff.id, 'assistantTeam', selectedTeam, true);
                                       
                                       // เลือก ERT อัตโนมัติตามทีม
                                       const defaultERT = getDefaultERTForTeam(selectedTeam);
                                       if (defaultERT) {
                                         updateSimpleTaskData(shift.id, staff.id, 'assistantErt', defaultERT, true);
                                       }
                                     }}
                                     className="task-select"
                                     style={{
