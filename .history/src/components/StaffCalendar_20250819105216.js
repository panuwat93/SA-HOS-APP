...
function StaffCalendar({ user }) {
  ...
  const renderShift = (date) => {
    ...
    if (day === 1) {
      ...
      console.log('🔍 Calendar - User info:', { id: user.id, uid: user.uid, firstName: user.firstName, lastName: user.lastName });
    }
      ...
      if (schedule.month === month && schedule.year === year && schedule.shifts) {
        ...
        if (dayShifts) {
          if (day === 1) { // แสดงเฉพาะวันแรก
            ...
            console.log(`🔍 Calendar - Day ${day} staffIds:`, Object.keys(dayShifts));
          }
          
          // ตรวจสอบว่า user.id ตรงกับ staffId ใดในตารางเวร
          Object.entries(dayShifts).forEach(([staffId, shiftData]) => {
            if (day === 1) { // แสดงเฉพาะวันแรก
              console.log(`🔍 Calendar - Checking staffId: ${staffId}, shiftData:`, shiftData);
            }
            ...
            if (typeof shiftData === 'string') {
              ...
              if (day === 1) { // แสดงเฉพาะวันแรก
                console.log(`🔍 Calendar - Parsed shift:`, parsedShift);
              }
              ...
              if (staffId === user.id || staffId === user.uid) {
                ...
                if (day === 1) { // แสดงเฉพาะวันแรก
                  console.log(`❌ Calendar - StaffId mismatch: ${staffId} vs ${user.id}/${user.uid}`);
                }
              }
            }
          });
          ...
          if (day === 1) { // แสดงเฉพาะวันแรก
            console.log(`❌ Calendar - No shifts for day ${day}`);
          }
        }
      }
    });
